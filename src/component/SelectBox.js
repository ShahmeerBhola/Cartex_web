...
export default function SelectBox({toggler,setToggler, toggle,data,topp,extraData}) {
  ...
  return (
    <div className='w-full '>
      <div className='  w-full'> 
      <div className="bg-transparent outline-0 w-full font-bold text-xs border-[#fff] text-[#1A7DC1] cursor-pointer" onClick={()=>setToggler(true)}>
      ...
      {
        toggler && ( <div className={`absolute flex flex-col z-10 inset-x-0 top-[80px] mt-2 w-full overflow-hidden  rounded-2xl text-xl font-normal shadow-2xl border-primary bg-white ${topp && 'top-[20px]'}`}>
          {
            toggle.map((item,i)=>(
                <>
